...
from fvd_utils.pytorch_i3d import InceptionI3d
...
def load_fvd_model(device):
    ...
    i3d.load_state_dict(torch.load("path_to/i3d_pretrained_400.pt", map_location=device))
    i3d.eval()
    return i3d
...
def frechet_distance(x1, x2):
    ...
    fd = trace + mean
    return fd.cpu().numpy(), F.cosine_similarity(x1, x2, dim=1).mean().cpu().numpy()
