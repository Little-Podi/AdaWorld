    ...
    try:
        ...
        if not opt.resume or opt.finetune:
            ...
            if len(missing) > 0 and (
                    (not opt.finetune or not os.path.exists(opt.finetune)) or "safetensors" in opt.finetune):
                model.reinit_ema()
                missing = [model_key for model_key in missing if "model_ema" not in model_key]
            print(f"Missing keys: {missing}")
            print_unexpected = [unexpected_k for unexpected_k in unexpected if
                                "conditioner.embedders.2." not in unexpected_k]
            print(f"Unexpected keys: {print_unexpected}")
